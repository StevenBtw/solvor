...
- **CP refactor**: Extracted SAT encoding logic to `cp_encoder.py` for better
  maintainability (cp.py went from 861 to 428 lines). No API changes. `Model.solve()` now defaults to `solver="auto"` which
  ...
  to force a specific solver. `IntVar` now supports multiplication with integers
